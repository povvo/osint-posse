...
recommended persistent investigation database. Green Ink is the workflow CLI;
its local task-state files are a limited continuity mechanism, not a database.
...
| Green Ink CLI state | Case workspace under `.green-ink/` | Progress is persisted to `ospo-db` during the normal workflow |
