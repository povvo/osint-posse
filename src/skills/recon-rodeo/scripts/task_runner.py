...
def validate_completion_manifest(
    ...
    for name, entry in supplied_scripts.items():
        ...
        _validate_evidence_list(entry.get("evidence"), f"Required script {name}", context)
    ...
    for name, entry in supplied_templates.items():
        ...
        for raw_path in output_paths:
            ...
            _resolve_evidence_file(raw_path, context)
    ...
    for name, entry in supplied_tools.items():
        ...
        _validate_evidence_list(entry.get("evidence"), f"Required tool {name}", context)
    ...
    _validate_evidence_list(notebook.get("evidence"), "Notebook update", context)
...
def cmd_done(
    ...
    try:
        completion_record = validate_completion_manifest(
            Path(evidence_manifest).expanduser().resolve(), task, state, context
        )
...
def cmd_reset(state: dict, context: CaseContext) -> None:
    """Reset progress for the selected investigation only."""
    state["completed"] = []
