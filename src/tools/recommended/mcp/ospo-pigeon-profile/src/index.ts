...
const PROFILE_FRAMEWORKS = FRAMEWORK_TABLE.filter(
  (entry) => entry.frameworkId !== "psychological-profiling-toolkit"
);
...
export class PsychologicalProfilingToolkit extends McpAgent<Env> {
  ...
  async init() {
    ...
    this.server.tool(
      ...
      {},
      async () => {
        const frameworks = PROFILE_FRAMEWORKS.map((entry) => {
    ...
    this.server.tool(
      ...
      async ({ problem, what_tried, success_looks_like }) => {
        ...
        const scores = PROFILE_FRAMEWORKS.map((entry) => {
          let score = 0;
