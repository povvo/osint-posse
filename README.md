...
Status: release candidate. The public source repository exists, while the first
intended package and GitHub release remain `0.1.0`. No PyPI availability or
support response window is claimed yet.
...
runtime contract. CI checks, tests, audits, and dry-run builds the applicable
Worker surfaces; it does not deploy them. `ospo-db` is the recommended
persistent investigation database: an authenticated 26-tool MCP service backed
by Cloudflare D1. Green Ink is the workflow CLI. Its small local task-state
files support CLI continuity when the Worker is not used; they are not a
database and do not replace the entity, relationship, evidence, grade,
timeline, location, or investigation records held by `ospo-db`.
...
ruff check ospo tests scripts setup.py
mypy ospo
python3 -m build --sdist
python3 -m pip wheel --no-deps --wheel-dir dist dist/*.tar.gz
