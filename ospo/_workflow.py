...
def _run_tool(
    ...
    completed = subprocess.run(
        [sys.executable, str(_scripts() / TOOLS[name]), *arguments],
        text=True,
        capture_output=True,
        check=False,
    )
    ...
    if receipt:
        if not investigation_id or not task_id:
            raise ValueError("--receipt requires --investigation-id and --task-id")
        destination = Path(receipt).expanduser().resolve()
        ...
        print(f"Receipt: {destination}", file=sys.stderr)
...
def build_parser(prog: str = "green-ink") -> argparse.ArgumentParser:
    ...
    run.add_argument("--investigation-id")
